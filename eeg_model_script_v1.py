...
# EEG Model Training Notebook Version 1 (original trail parameters)
# 
...
def main_runner(subject_choice=1, label_type='angle'):
    ...
    for cur_k in range(len(indices)):
        ...
        classifier_model = lstm_classifier(num_features=num_features, num_hidden=classifier_hidden, dropout=0.9, n_labels=num_classes)
        ...
        classifier_train_metrics = train_classifier(classifier_model, classifier_train_epochs, batch_size=64, learning_rate=classifier_learning_rate, features=encoded_train_features, labels=lstm_train_labels, num_classes=num_classes)
...
def run():
    subjects = [(x+1) for x in range(15)]
    t0 = time.time()
    label_type = 'both'
    result_list = np.vstack([main_runner(subject, label_type) for subject in tqdm(subjects)])
    # result_list_pos = np.vstack([main_runner(subject, 'pos') for subject in tqdm(subjects)])
    # results_list_angle = np.vstack([main_runner(subject, 'angle') for subject in tqdm(subjects)])
    #result_list = np.vstack([result_list_pos, results_list_angle])
    t1 = time.time()
